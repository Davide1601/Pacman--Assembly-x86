TITLE Pacman 2.5, Chris Cullati Bryan Quinlan, November 5, 2013				(main.asm)

; 
; 
; 
; 
; Revision date:

INCLUDE Irvine32.inc
INCLUDE mCheckRow.txt

.data

champ_x	BYTE 23						;Pacman's x coordinate on the console
champ_xnew BYTE 23					;when a direction is pressed, champ_xnew holds the x coordinate pacman is trying to enter
champ_y BYTE 14						;Pacman's y coordinate
champ_ynew BYTE 14					;Champ_xnew : x coordinate :: champ_ynew : y coordinate (thats an analogy)
champ BYTE '<'						;Represents pacman (initially facing right)
char BYTE ?							;A placeholder variable
lives BYTE 3						;Represents Pacman's remaining lives
powermode BYTE 0					;Boolean, 1 means powermode on, 0 means off
powercount BYTE 0					;How many updates left until powermode ends
POWER_MAX db 100					;Max powermode time, decreases as level increases
dots BYTE 0							;How many dots pacman has eaten this level, bonus fruit is dependent on this
level db 1							;Represents what Level you are on
bonus dw 0							;how much the bonus is worth
hasbonuslife db 0					;boolean for having bonus life or not

pauseme db 0						;Basically a boolean, 1 means game is paused, 0 means game is unpaused
pausescreen_msg BYTE "**PAUSED**", 0

score WORD 0						;Player's score
scoreMod WORD 10					;The number by which score is incremented
score_msg BYTE "Score: ",0			;Message for outputting score
level_msg BYTE "Level: ",0
lives_msg BYTE "Lives: ",0			;I'd like to implement this differently, with the lives indicatd by little pacmen at the bottom, but for now this works

mapwdots0 BYTE	"############################", 0
mapwdots1 BYTE	"#............##............#", 0
mapwdots2 BYTE	"#.####.#####.##.#####.####.#", 0
mapwdots3 BYTE	"#*#  #.#   #.##.#   #.#  #*#", 0
mapwdots4 BYTE	"#.####.#####.##.#####.####.#", 0
mapwdots5 BYTE	"#..........................#", 0
mapwdots6 BYTE	"#.####.##.########.##.####.#", 0
mapwdots7 BYTE	"#.####.##.########.##.####.#", 0
mapwdots8 BYTE	"#......##....##....##......#", 0
mapwdots9 BYTE	"######.##### ## #####.######", 0
mapwdots10 BYTE	"     #.##### ## #####.#     ", 0
mapwdots11 BYTE	"     #.##    @     ##.#     ", 0
mapwdots12 BYTE	"     #.## ###--### ##.#     ", 0
mapwdots13 BYTE	"######.## #      # ##.######", 0
mapwdots14 BYTE	"      .   #      #   .      ", 0
mapwdots15 BYTE	"######.## # @ @ @# ##.######", 0
mapwdots16 BYTE	"     #.## ######## ##.#     ", 0
mapwdots17 BYTE	"     #.##          ##.#     ", 0
mapwdots18 BYTE	"     #.## ######## ##.#     ", 0
mapwdots19 BYTE	"######.## ######## ##.######", 0
mapwdots20 BYTE	"#............##............#", 0
mapwdots21 BYTE	"#.####.#####.##.#####.####.#", 0
mapwdots22 BYTE	"#.####.#####.##.#####.####.#", 0
mapwdots23 BYTE	"#*..##........ .......##..*#", 0
mapwdots24 BYTE	"###.##.##.########.##.##.###", 0
mapwdots25 BYTE	"###.##.##.########.##.##.###", 0
mapwdots26 BYTE	"#......##....##....##......#", 0
mapwdots27 BYTE	"#.##########.##.##########.#", 0
mapwdots28 BYTE	"#.##########.##.##########.#", 0
mapwdots29 BYTE "#..........................#", 0
mapwdots30 BYTE	"############################", 0


.code

main PROC
	Update:
		call UpdateChamp					;Update function, checks keys and alters rest of program appropriately
		cmp champ_X, -1						;Check if q was pressed (quits if so)
		je exitron
		cmp pauseme, 1						;check if p was pressed (pauses if so)
		je paused
		call CheckMove						;runs proc to see if new move generated by UpdateChamp is legal, allows if legal, disallows if illegal
		call PrintMapWithDots				;prints the map
		call WriteChamp						;prints Pacman
		call CheckDots						;checks stuff related to score and drawing of bonus fruit
		mov dh, 0
		mov dl, 31
		call GoToXY
		mov edx, OFFSET level_msg			;level output on the side of the game
		call WriteString
		mov al, level
		call WriteDec
		mov dh, 1
		mov dl, 31
		call GoToXY
		mov edx, OFFSET lives_msg			;lives output on side
		call WriteString
		mov al, lives
		call WriteDec
		mov dh, 2
		mov dl, 31
		call GoToXY
		mov edx, OFFSET score_msg			;score output on side
		call WriteString
		mov ax, score
		call WriteDec
		call CheckScore						;checks to see whether or not bonus life should be added
		mov al, champ_x						;super important
		mov bl, champ_y						;Sets champ_x/ynew
		mov champ_xnew, al					;to champ_x/y
		mov champ_ynew, bl					;without this, one could keep pressing left against a wall until new_y = a legal value then pac would teleport there
		mov eax, 100						;after testing, delay of 200 seems to work best with regards to screen flicker and FPS
		call Delay
		call Clrscr							;clear the screen and update, I will test this later to see if it is even necessary (I think it will be)
		jmp Update							;infinite loop unless quit or pause (really just quit)

	paused:									;jumps here when paused to loop until unpaused
		call UpdatePause					;check basically just to see if pause has been pressed again
		call PauseScreen					;display pause screen
		cmp pauseme, 0						;check pause value
		
		je Update
		jne paused
	exitron:								;end of program. This is where we will implement game over screens, highscores, and any other extras. also additional levels
		exit

main ENDP

PrintMapWithDots PROC

mov dl, 0
mov dh, 0
call GoToXY

;Print row 0

mov eax, 1
call SetTextColor
mov edx, OFFSET mapwdots0
call WriteString
call Crlf

;Print row 1

mov esi, 0

mov al, mapwdots1[esi]
call Writechar
inc esi

mov eax, 15
call SetTextColor

LoopRow11:
	
	mov al, mapwdots1[esi]
	call WriteChar
	inc esi
	cmp esi, 13
	jl LoopRow11

mov eax, 1
call SetTextColor

LoopRow12:
	mov al, mapwdots1[esi]
	call Writechar
	inc esi
	cmp esi, 15
	jl LoopRow12

mov eax, 15
call SetTextColor

LoopRow13:
	mov al, mapwdots1[esi]
	call WriteChar
	inc esi
	cmp esi, 27
	jl LoopRow13

mov eax, 1
call SetTextColor
mov al, mapwdots1[esi]
call Writechar
call Crlf

;Print row 2

mov esi, 0

mov al, mapwdots2[esi]
call WriteChar
inc esi
mov eax, 15
call SetTextColor
mov al, mapwdots2[esi]
call Writechar
inc esi
mov eax, 1
call SetTextColor

LoopRow21:
	mov al, mapwdots2[esi]
	call Writechar
	inc esi
	cmp esi, 6
	jl LoopRow21

mov eax, 15
call SetTextColor
mov al, mapwdots2[esi]
call WriteChar
inc esi
mov eax, 1
call SetTextColor

LoopRow22:
	mov al, mapwdots2[esi]
	call WriteChar
	inc esi
	cmp esi, 12
	jl LoopRow22

mov eax, 15
call SetTextColor
mov al, mapwdots2[esi]
call WriteChar
inc esi
mov eax, 1
call SetTextColor

LoopRow23:
	mov al, mapwdots2[esi]
	call WriteChar
	inc esi
	cmp esi, 15
	jl LoopRow23

mov eax, 15
call SetTextColor
mov al, mapwdots2[esi]
call WriteChar
inc esi
mov eax, 1
call SetTextColor

LoopRow24:
	mov al, mapwdots2[esi]
	call WriteChar
	inc esi
	cmp esi, 21
	jl LoopRow24

mov eax, 15
call SetTextColor
mov al, mapwdots2[esi]
call WriteChar
inc esi
mov eax, 1
call SetTextColor

LoopRow25:
	mov al, mapwdots2[esi]
	call WriteChar
	inc esi
	cmp esi, 26
	jl LoopRow25

mov eax, 15
call SetTextColor
mov al, mapwdots2[esi]
call WriteChar
inc esi
mov eax, 1
call SetTextColor
mov al, mapwdots2[esi]
call Writechar
call Crlf

;Print row 3

mov esi, 0

mov al, mapwdots3[esi]
call WriteChar
inc esi
mov eax, 15
call SetTextColor
mov al, mapwdots3[esi]
call Writechar
inc esi
mov eax, 1
call SetTextColor

LoopRow31:
	mov al, mapwdots3[esi]
	call Writechar
	inc esi
	cmp esi, 6
	jl LoopRow31

mov eax, 15
call SetTextColor
mov al, mapwdots3[esi]
call WriteChar
inc esi
mov eax, 1
call SetTextColor

LoopRow32:
	mov al, mapwdots3[esi]
	call WriteChar
	inc esi
	cmp esi, 12
	jl LoopRow32

mov eax, 15
call SetTextColor
mov al, mapwdots3[esi]
call WriteChar
inc esi
mov eax, 1
call SetTextColor

LoopRow33:
	mov al, mapwdots3[esi]
	call WriteChar
	inc esi
	cmp esi, 15
	jl LoopRow33

mov eax, 15
call SetTextColor
mov al, mapwdots3[esi]
call WriteChar
inc esi
mov eax, 1
call SetTextColor

LoopRow34:
	mov al, mapwdots3[esi]
	call WriteChar
	inc esi
	cmp esi, 21
	jl LoopRow34

mov eax, 15
call SetTextColor
mov al, mapwdots3[esi]
call WriteChar
inc esi
mov eax, 1
call SetTextColor

LoopRow35:
	mov al, mapwdots3[esi]
	call WriteChar
	inc esi
	cmp esi, 26
	jl LoopRow35

mov eax, 15
call SetTextColor
mov al, mapwdots3[esi]
call WriteChar
inc esi
mov eax, 1
call SetTextColor
mov al, mapwdots3[esi]
call Writechar
call Crlf

;Print row 4

mov esi, 0

mov al, mapwdots4[esi]
call WriteChar
inc esi
mov eax, 15
call SetTextColor
mov al, mapwdots4[esi]
call Writechar
inc esi
mov eax, 1
call SetTextColor

LoopRow41:
	mov al, mapwdots4[esi]
	call Writechar
	inc esi
	cmp esi, 6
	jl LoopRow41

mov eax, 15
call SetTextColor
mov al, mapwdots4[esi]
call WriteChar
inc esi
mov eax, 1
call SetTextColor

LoopRow42:
	mov al, mapwdots4[esi]
	call WriteChar
	inc esi
	cmp esi, 12
	jl LoopRow42

mov eax, 15
call SetTextColor
mov al, mapwdots4[esi]
call WriteChar
inc esi
mov eax, 1
call SetTextColor

LoopRow43:
	mov al, mapwdots4[esi]
	call WriteChar
	inc esi
	cmp esi, 15
	jl LoopRow43

mov eax, 15
call SetTextColor
mov al, mapwdots4[esi]
call WriteChar
inc esi
mov eax, 1
call SetTextColor

LoopRow44:
	mov al, mapwdots4[esi]
	call WriteChar
	inc esi
	cmp esi, 21
	jl LoopRow44

mov eax, 15
call SetTextColor
mov al, mapwdots4[esi]
call WriteChar
inc esi
mov eax, 1
call SetTextColor

LoopRow45:
	mov al, mapwdots4[esi]
	call WriteChar
	inc esi
	cmp esi, 26
	jl LoopRow45

mov eax, 15
call SetTextColor
mov al, mapwdots4[esi]
call WriteChar
inc esi
mov eax, 1
call SetTextColor
mov al, mapwdots4[esi]
call Writechar
call Crlf

;Print row 5

mov esi, 0

mov al, mapwdots5[esi]
call Writechar
inc esi
mov eax, 15
call SetTextColor

LoopRow51:
	mov al, mapwdots5[esi]
	call Writechar
	inc esi
	cmp esi, 27
	jl LoopRow51

mov eax, 1
call SetTextColor
mov al, mapwdots5[esi]
call Writechar
call Crlf

;Print row 6

mov esi, 0
mov al, mapwdots6[esi]
call Writechar
inc esi
mov eax, 15
call SetTextColor
mov al, mapwdots6[esi]
call Writechar
inc esi
mov eax, 1
call SetTextColor

LoopRow61:
	mov al, mapwdots6[esi]
	call Writechar
	inc esi
	cmp esi, 6
	jl LoopRow61

mov eax, 15
call SetTextColor
mov al, mapwdots6[esi]
call Writechar
inc esi
mov eax, 1
call SetTextColor

Loop62:
	mov al, mapwdots6[esi]
	call Writechar
	inc esi
	cmp esi, 9
	jl Loop62

mov eax, 15
call SetTextColor
mov al, mapwdots6[esi]
call WriteChar
inc esi
mov eax, 1
call SetTextColor

LoopRow63:
	mov al, mapwdots6[esi]
	call Writechar
	inc esi
	cmp esi, 18
	jl LoopRow63

mov eax, 15
call SetTextColor
mov al, mapwdots6[esi]
call Writechar
inc esi
mov eax, 1
call SetTextColor

LoopRow64:
	mov al, mapwdots6[esi]
	call WriteChar
	inc esi
	cmp esi, 21
	jl LoopRow64

mov eax, 15
call SetTextColor
mov al, mapwdots6[esi]
call Writechar
inc esi
mov eax, 1
call SetTextColor

LoopRow65:
	mov al, mapwdots6[esi]
	call Writechar
	inc esi
	cmp esi, 26
	jl LoopRow65

mov eax, 15
call SetTextColor
mov al, mapwdots6[esi]
call Writechar
inc esi
mov eax, 1
call SetTextColor
mov al, mapwdots6[esi]
call Writechar
call Crlf

;Print row 7

mov esi, 0
mov al, mapwdots7[esi]
call Writechar
inc esi
mov eax, 15
call SetTextColor
mov al, mapwdots7[esi]
call Writechar
inc esi
mov eax, 1
call SetTextColor

LoopRow71:
	mov al, mapwdots7[esi]
	call Writechar
	inc esi
	cmp esi, 6
	jl LoopRow71

mov eax, 15
call SetTextColor
mov al, mapwdots7[esi]
call Writechar
inc esi
mov eax, 1
call SetTextColor

Loop72:
	mov al, mapwdots7[esi]
	call Writechar
	inc esi
	cmp esi, 9
	jl Loop72

mov eax, 15
call SetTextColor
mov al, mapwdots7[esi]
call WriteChar
inc esi
mov eax, 1
call SetTextColor

LoopRow73:
	mov al, mapwdots7[esi]
	call Writechar
	inc esi
	cmp esi, 18
	jl LoopRow73

mov eax, 15
call SetTextColor
mov al, mapwdots7[esi]
call Writechar
inc esi
mov eax, 1
call SetTextColor

LoopRow74:
	mov al, mapwdots7[esi]
	call WriteChar
	inc esi
	cmp esi, 21
	jl LoopRow74

mov eax, 15
call SetTextColor
mov al, mapwdots7[esi]
call Writechar
inc esi
mov eax, 1
call SetTextColor

LoopRow75:
	mov al, mapwdots7[esi]
	call Writechar
	inc esi
	cmp esi, 26
	jl LoopRow75

mov eax, 15
call SetTextColor
mov al, mapwdots7[esi]
call Writechar
inc esi
mov eax, 1
call SetTextColor
mov al, mapwdots7[esi]
call Writechar
call Crlf

;Print row 8

mov esi, 0

mov al, mapwdots8[esi]
call Writechar
inc esi
mov eax, 15
call SetTextColor

LoopRow81:
	mov al, mapwdots8[esi]
	call Writechar
	inc esi
	cmp esi, 7
	jl LoopRow81

mov eax, 1
call SetTextColor

LoopRow82:
	mov al, mapwdots8[esi]
	call Writechar
	inc esi
	cmp esi, 9
	jl LoopRow82

mov eax, 15
call SetTextColor

LoopRow83:
	mov al, mapwdots8[esi]
	call Writechar
	inc esi
	cmp esi, 13
	jl LoopRow83

mov eax, 1
call SetTextColor

LoopRow84:
	mov al, mapwdots8[esi]
	call Writechar
	inc esi
	cmp esi, 15
	jl LoopRow84

mov eax, 15
call SetTextColor

LoopRow85:
	mov al, mapwdots8[esi]
	call Writechar
	inc esi
	cmp esi, 19
	jl LoopRow85

mov eax, 1
call SetTextColor

LoopRow86:
	mov al, mapwdots8[esi]
	call Writechar
	inc esi
	cmp esi, 21
	jl LoopRow86

mov eax, 15
call SetTextColor

LoopRow87:
	mov al, mapwdots8[esi]
	call Writechar
	inc esi
	cmp esi, 27
	jl LoopRow87

mov eax, 1
call SetTextColor
mov al, mapwdots8[esi]
call Writechar
call Crlf

;Print row 9

mov esi, 0

LoopRow91:
	mov al, mapwdots9[esi]
	call Writechar
	inc esi
	cmp esi, 6
	jl LoopRow91

mov eax, 15
call SetTextColor
mov al, mapwdots9[esi]
call Writechar
inc esi
mov eax, 1
call SetTextColor

LoopRow92:
	mov al, mapwdots9[esi]
	call Writechar
	inc esi
	cmp esi, 21
	jl LoopRow92

mov eax, 15
call SetTextColor
mov al, mapwdots9[esi]
call Writechar
inc esi
mov eax, 1
call SetTextColor

LoopRow93:
	mov al, mapwdots9[esi]
	call Writechar
	inc esi
	cmp esi, 28
	jl LoopRow93

call Crlf

;Print row 10

mov esi, 0

LoopRow101:
	mov al, mapwdots10[esi]
	call Writechar
	inc esi
	cmp esi, 6
	jl LoopRow101

mov eax, 15
call SetTextColor
mov al, mapwdots10[esi]
call Writechar
inc esi
mov eax, 1
call SetTextColor

LoopRow102:
	mov al, mapwdots10[esi]
	call Writechar
	inc esi
	cmp esi, 21
	jl LoopRow102

mov eax, 15
call SetTextColor
mov al, mapwdots10[esi]
call Writechar
inc esi
mov eax, 1
call SetTextColor

LoopRow103:
	mov al, mapwdots10[esi]
	call Writechar
	inc esi
	cmp esi, 28
	jl LoopRow103

call Crlf

;Print row 11

mov esi, 0

LoopRow111:
	mov al, mapwdots11[esi]
	call Writechar
	inc esi
	cmp esi, 6
	jl LoopRow111

mov eax, 15
call SetTextColor
mov al, mapwdots11[esi]
call Writechar
inc esi
mov eax, 1
call SetTextColor

LoopRow112:
	mov al, mapwdots11[esi]
	call Writechar
	inc esi
	cmp esi, 13
	jl LoopRow112

mov eax, 12
call SetTextColor

LoopRow113:
	mov al, mapwdots11[esi]
	call Writechar
	inc esi
	cmp esi, 15
	jl LoopRow113

mov eax, 1
call SetTextColor

LoopRow114:
	mov al, mapwdots11[esi]
	call Writechar
	inc esi
	cmp esi, 21
	jl LoopRow114

mov eax, 15
call SetTextColor
mov al, mapwdots11[esi]
call Writechar
inc esi
mov eax, 1
call SetTextColor

LoopRow115:
	mov al, mapwdots11[esi]
	call Writechar
	inc esi
	cmp esi, 28
	jl LoopRow115

call Crlf

;Print row 12

mov esi, 0

LoopRow121:
	mov al, mapwdots12[esi]
	call Writechar
	inc esi
	cmp esi, 6
	jl LoopRow121

mov eax, 15
call SetTextColor
mov al, mapwdots12[esi]
call Writechar
inc esi
mov eax, 1
call SetTextColor

LoopRow122:
	mov al, mapwdots12[esi]
	call Writechar
	inc esi
	cmp esi, 13
	jl LoopRow122

mov eax, 5
call SetTextColor

LoopRow123:
	mov al, mapwdots12[esi]
	call Writechar
	inc esi
	cmp esi, 15
	jl LoopRow123

mov eax, 1
call SetTextColor

LoopRow124:
	mov al, mapwdots12[esi]
	call Writechar
	inc esi
	cmp esi, 21
	jl LoopRow124

mov eax, 15
call SetTextColor
mov al, mapwdots12[esi]
call Writechar
inc esi
mov eax, 1
call SetTextColor

LoopRow125:
	mov al, mapwdots12[esi]
	call Writechar
	inc esi
	cmp esi, 28
	jl LoopRow125

call Crlf

;Print row 13

mov esi, 0

LoopRow131:
	mov al, mapwdots13[esi]
	call Writechar
	inc esi
	cmp esi, 6
	jl LoopRow131

mov eax, 15
call SetTextColor
mov al, mapwdots13[esi]
call Writechar
inc esi

mov eax, 1
call SetTextColor

LoopRow132:
	mov al, mapwdots13[esi]
	call Writechar
	inc esi
	cmp esi, 21
	jl LoopRow132

mov eax, 15
call SetTextColor
mov al, mapwdots13[esi]
call Writechar
inc esi
mov eax, 1
call SetTextColor

LoopRow133:
	mov al, mapwdots13[esi]
	call Writechar
	inc esi
	cmp esi, 28
	jl LoopRow133

call Crlf

;Print row 14

mov esi, 0

LoopRow141:
	mov al, mapwdots14[esi]
	call Writechar
	inc esi
	cmp esi, 6
	jl LoopRow141

mov eax, 15
call SetTextColor
mov al, mapwdots14[esi]
call Writechar
inc esi

mov eax, 1
call SetTextColor

LoopRow142:
	mov al, mapwdots14[esi]
	call Writechar
	inc esi
	cmp esi, 21
	jl LoopRow142

mov eax, 15
call SetTextColor
mov al, mapwdots14[esi]
call Writechar
inc esi
mov eax, 1
call SetTextColor

LoopRow143:
	mov al, mapwdots14[esi]
	call Writechar
	inc esi
	cmp esi, 28
	jl LoopRow143

call Crlf

;Print row 15

mov esi, 0

LoopRow151:
	mov al, mapwdots15[esi]
	call Writechar
	inc esi
	cmp esi, 6
	jl LoopRow151

mov eax, 15
call SetTextColor
mov al, mapwdots15[esi]
call Writechar
inc esi

mov eax, 1
call SetTextColor

LoopRow152:
	mov al, mapwdots15[esi]
	call Writechar
	inc esi
	cmp esi, 12
	jl LoopRow152

mov eax, 11
call SetTextColor

LoopRow153:
	mov al, mapwdots15[esi]
	call Writechar
	inc esi
	cmp esi, 14
	jl LoopRow153

mov eax, 13
call SetTextColor

LoopRow154:
	mov al, mapwdots15[esi]
	call Writechar
	inc esi
	cmp esi, 16
	jl LoopRow154

mov eax, 10
call SetTextColor
mov al, mapwdots15[esi]
call WriteChar
inc esi
mov eax, 1
call SetTextColor

LoopRow155:
	mov al, mapwdots15[esi]
	call Writechar
	inc esi
	cmp esi, 21
	jl LoopRow155

mov eax, 15
call SetTextColor
mov al, mapwdots15[esi]
call Writechar
inc esi
mov eax, 1
call SetTextColor

LoopRow156:
	mov al, mapwdots15[esi]
	call Writechar
	inc esi
	cmp esi, 28
	jl LoopRow156

call Crlf

;Print row 16

mov esi, 0

LoopRow161:
	mov al, mapwdots16[esi]
	call Writechar
	inc esi
	cmp esi, 6
	jl LoopRow161

mov eax, 15
call SetTextColor
mov al, mapwdots16[esi]
call Writechar
inc esi

mov eax, 1
call SetTextColor

LoopRow162:
	mov al, mapwdots16[esi]
	call Writechar
	inc esi
	cmp esi, 21
	jl LoopRow162

mov eax, 15
call SetTextColor
mov al, mapwdots16[esi]
call Writechar
inc esi
mov eax, 1
call SetTextColor

LoopRow163:
	mov al, mapwdots16[esi]
	call Writechar
	inc esi
	cmp esi, 28
	jl LoopRow163

call Crlf

;Print row 17

mov esi, 0

LoopRow171:
	mov al, mapwdots17[esi]
	call Writechar
	inc esi
	cmp esi, 6
	jl LoopRow171

mov eax, 15
call SetTextColor
mov al, mapwdots17[esi]
call Writechar
inc esi

mov eax, 1
call SetTextColor

LoopRow172:
	mov al, mapwdots17[esi]
	call Writechar
	inc esi
	cmp esi, 21
	jl LoopRow172

mov eax, 15
call SetTextColor
mov al, mapwdots17[esi]
call Writechar
inc esi
mov eax, 1
call SetTextColor

LoopRow173:
	mov al, mapwdots17[esi]
	call Writechar
	inc esi
	cmp esi, 28
	jl LoopRow173

call Crlf

;Print row 18

mov esi, 0

LoopRow181:
	mov al, mapwdots18[esi]
	call Writechar
	inc esi
	cmp esi, 6
	jl LoopRow181

mov eax, 15
call SetTextColor
mov al, mapwdots18[esi]
call Writechar
inc esi

mov eax, 1
call SetTextColor

LoopRow182:
	mov al, mapwdots18[esi]
	call Writechar
	inc esi
	cmp esi, 21
	jl LoopRow182

mov eax, 15
call SetTextColor
mov al, mapwdots18[esi]
call Writechar
inc esi
mov eax, 1
call SetTextColor

LoopRow183:
	mov al, mapwdots18[esi]
	call Writechar
	inc esi
	cmp esi, 28
	jl LoopRow183

call Crlf

;Print row 19

mov esi, 0

LoopRow191:
	mov al, mapwdots19[esi]
	call Writechar
	inc esi
	cmp esi, 6
	jl LoopRow191

mov eax, 15
call SetTextColor
mov al, mapwdots19[esi]
call Writechar
inc esi

mov eax, 1
call SetTextColor

LoopRow192:
	mov al, mapwdots19[esi]
	call Writechar
	inc esi
	cmp esi, 21
	jl LoopRow192

mov eax, 15
call SetTextColor
mov al, mapwdots19[esi]
call Writechar
inc esi
mov eax, 1
call SetTextColor

LoopRow193:
	mov al, mapwdots19[esi]
	call Writechar
	inc esi
	cmp esi, 28
	jl LoopRow193

call Crlf

;Print row 20

mov esi, 0

mov al, mapwdots20[esi]
call Writechar
inc esi

mov eax, 15
call SetTextColor

LoopRow201:
	
	mov al, mapwdots20[esi]
	call WriteChar
	inc esi
	cmp esi, 13
	jl LoopRow201

mov eax, 1
call SetTextColor

LoopRow202:
	mov al, mapwdots1[esi]
	call Writechar
	inc esi
	cmp esi, 15
	jl LoopRow202

mov eax, 15
call SetTextColor

LoopRow203:
	mov al, mapwdots20[esi]
	call WriteChar
	inc esi
	cmp esi, 27
	jl LoopRow203

mov eax, 1
call SetTextColor
mov al, mapwdots20[esi]
call Writechar
call Crlf

;Print row 21

mov esi, 0

mov al, mapwdots21[esi]
call WriteChar
inc esi
mov eax, 15
call SetTextColor
mov al, mapwdots21[esi]
call Writechar
inc esi
mov eax, 1
call SetTextColor

LoopRow211:
	mov al, mapwdots21[esi]
	call Writechar
	inc esi
	cmp esi, 6
	jl LoopRow211

mov eax, 15
call SetTextColor
mov al, mapwdots21[esi]
call WriteChar
inc esi
mov eax, 1
call SetTextColor

LoopRow212:
	mov al, mapwdots21[esi]
	call WriteChar
	inc esi
	cmp esi, 12
	jl LoopRow212

mov eax, 15
call SetTextColor
mov al, mapwdots21[esi]
call WriteChar
inc esi
mov eax, 1
call SetTextColor

LoopRow213:
	mov al, mapwdots21[esi]
	call WriteChar
	inc esi
	cmp esi, 15
	jl LoopRow213

mov eax, 15
call SetTextColor
mov al, mapwdots21[esi]
call WriteChar
inc esi
mov eax, 1
call SetTextColor

LoopRow214:
	mov al, mapwdots21[esi]
	call WriteChar
	inc esi
	cmp esi, 21
	jl LoopRow214

mov eax, 15
call SetTextColor
mov al, mapwdots21[esi]
call WriteChar
inc esi
mov eax, 1
call SetTextColor

LoopRow215:
	mov al, mapwdots21[esi]
	call WriteChar
	inc esi
	cmp esi, 26
	jl LoopRow215

mov eax, 15
call SetTextColor
mov al, mapwdots21[esi]
call WriteChar
inc esi
mov eax, 1
call SetTextColor
mov al, mapwdots21[esi]
call Writechar
call Crlf

;Print row 2

mov esi, 0

mov al, mapwdots22[esi]
call WriteChar
inc esi
mov eax, 15
call SetTextColor
mov al, mapwdots22[esi]
call Writechar
inc esi
mov eax, 1
call SetTextColor

LoopRow221:
	mov al, mapwdots22[esi]
	call Writechar
	inc esi
	cmp esi, 6
	jl LoopRow221

mov eax, 15
call SetTextColor
mov al, mapwdots22[esi]
call WriteChar
inc esi
mov eax, 1
call SetTextColor

LoopRow222:
	mov al, mapwdots22[esi]
	call WriteChar
	inc esi
	cmp esi, 12
	jl LoopRow222

mov eax, 15
call SetTextColor
mov al, mapwdots22[esi]
call WriteChar
inc esi
mov eax, 1
call SetTextColor

LoopRow223:
	mov al, mapwdots22[esi]
	call WriteChar
	inc esi
	cmp esi, 15
	jl LoopRow223

mov eax, 15
call SetTextColor
mov al, mapwdots22[esi]
call WriteChar
inc esi
mov eax, 1
call SetTextColor

LoopRow224:
	mov al, mapwdots22[esi]
	call WriteChar
	inc esi
	cmp esi, 21
	jl LoopRow224

mov eax, 15
call SetTextColor
mov al, mapwdots22[esi]
call WriteChar
inc esi
mov eax, 1
call SetTextColor

LoopRow225:
	mov al, mapwdots22[esi]
	call WriteChar
	inc esi
	cmp esi, 26
	jl LoopRow225

mov eax, 15
call SetTextColor
mov al, mapwdots22[esi]
call WriteChar
inc esi
mov eax, 1
call SetTextColor
mov al, mapwdots22[esi]
call Writechar
call Crlf

;Prints row 23

mov esi, 0

mov al, mapwdots23[esi]
call Writechar
inc esi
mov eax, 15
call SetTextColor

LoopRow231:
	mov al, mapwdots23[esi]
	call Writechar
	inc esi
	cmp esi, 4
	jl LoopRow231

mov eax, 1
call SetTextColor

LoopRow232:
	mov al, mapwdots23[esi]
	call Writechar
	inc esi
	cmp esi, 6
	jl LoopRow232

mov eax, 15
call SetTextColor

LoopRow233:
	mov al, mapwdots23[esi]
	call Writechar
	inc esi
	cmp esi, 14
	jl LoopRow233

mov eax, 14
call SetTextColor
mov al, mapwdots23[esi]
call Writechar
inc esi
mov eax, 15
call SetTextColor

LoopRow234:
	mov al, mapwdots23[esi]
	call Writechar
	inc esi
	cmp esi, 22
	jl LoopRow234

mov eax, 1
call SetTextColor

LoopRow235:
	mov al, mapwdots23[esi]
	call Writechar
	inc esi
	cmp esi, 24
	jl LoopRow235

mov eax, 15
call SetTextColor

LoopRow236:
	mov al, mapwdots23[esi]
	call Writechar
	inc esi
	cmp esi, 27
	jl LoopRow236

mov eax, 1
call SetTextColor
mov al, mapwdots23[esi]
call Writechar
call Crlf

;Print row 24

mov esi, 0

LoopRow241:
	mov al, mapwdots24[esi]
	call Writechar
	inc esi
	cmp esi, 3
	jl LoopRow241

mov eax, 15
call SetTextColor
mov al, mapwdots24[esi]
call Writechar
inc esi
mov eax, 1
call SetTextColor

LoopRow242:
	mov al, mapwdots24[esi]
	call Writechar
	inc esi
	cmp esi, 6
	jl LoopRow242

mov eax, 15
call SetTextColor
mov al, mapwdots24[esi]
call Writechar
inc esi
mov eax, 1
call SetTextColor

LoopRow243:
	mov al, mapwdots24[esi]
	call Writechar
	inc esi
	cmp esi, 9
	jl LoopRow243

mov eax, 15
call SetTextColor
mov al, mapwdots24[esi]
call Writechar
inc esi
mov eax, 1
call SetTextColor

LoopRow244:
	mov al, mapwdots24[esi]
	call Writechar
	inc esi
	cmp esi, 18
	jl LoopRow244

mov eax, 15
call SetTextColor
mov al, mapwdots24[esi]
call Writechar
inc esi
mov eax, 1
call SetTextColor

LoopRow245:
	mov al, mapwdots24[esi]
	call Writechar
	inc esi
	cmp esi, 21
	jl LoopRow245

mov eax, 15
call SetTextColor
mov al, mapwdots24[esi]
call Writechar
inc esi
mov eax, 1
call SetTextColor

LoopRow246:
	mov al, mapwdots24[esi]
	call Writechar
	inc esi
	cmp esi, 24
	jl LoopRow246

mov eax, 15
call SetTextColor
mov al, mapwdots24[esi]
call Writechar
inc esi
mov eax, 1
call SetTextColor

LoopRow247:
	mov al, mapwdots24[esi]
	call Writechar
	inc esi
	cmp esi, 28
	jl LoopRow247

call Crlf

;Print row 25

mov esi, 0

LoopRow251:
	mov al, mapwdots25[esi]
	call Writechar
	inc esi
	cmp esi, 3
	jl LoopRow251

mov eax, 15
call SetTextColor
mov al, mapwdots25[esi]
call Writechar
inc esi
mov eax, 1
call SetTextColor

LoopRow252:
	mov al, mapwdots25[esi]
	call Writechar
	inc esi
	cmp esi, 6
	jl LoopRow252

mov eax, 15
call SetTextColor
mov al, mapwdots25[esi]
call Writechar
inc esi
mov eax, 1
call SetTextColor

LoopRow253:
	mov al, mapwdots25[esi]
	call Writechar
	inc esi
	cmp esi, 9
	jl LoopRow253

mov eax, 15
call SetTextColor
mov al, mapwdots25[esi]
call Writechar
inc esi
mov eax, 1
call SetTextColor

LoopRow254:
	mov al, mapwdots25[esi]
	call Writechar
	inc esi
	cmp esi, 18
	jl LoopRow254

mov eax, 15
call SetTextColor
mov al, mapwdots25[esi]
call Writechar
inc esi
mov eax, 1
call SetTextColor

LoopRow255:
	mov al, mapwdots25[esi]
	call Writechar
	inc esi
	cmp esi, 21
	jl LoopRow255

mov eax, 15
call SetTextColor
mov al, mapwdots25[esi]
call Writechar
inc esi
mov eax, 1
call SetTextColor

LoopRow256:
	mov al, mapwdots25[esi]
	call Writechar
	inc esi
	cmp esi, 24
	jl LoopRow256

mov eax, 15
call SetTextColor
mov al, mapwdots25[esi]
call Writechar
inc esi
mov eax, 1
call SetTextColor

LoopRow257:
	mov al, mapwdots25[esi]
	call Writechar
	inc esi
	cmp esi, 28
	jl LoopRow257

call Crlf

;Print row 26

mov esi, 0

mov al, mapwdots26[esi]
call Writechar
inc esi
mov eax, 15
call SetTextColor

LoopRow261:
	mov al, mapwdots26[esi]
	call Writechar
	inc esi
	cmp esi, 7
	jl LoopRow261

mov eax, 1
call SetTextColor

LoopRow262:
	mov al, mapwdots26[esi]
	call Writechar
	inc esi
	cmp esi, 9
	jl LoopRow262

mov eax, 15
call SetTextColor

LoopRow263:
	mov al, mapwdots26[esi]
	call Writechar
	inc esi
	cmp esi, 13
	jl LoopRow263

mov eax, 1
call SetTextColor

LoopRow264:
	mov al, mapwdots26[esi]
	call Writechar
	inc esi
	cmp esi, 15
	jl LoopRow264

mov eax, 15
call SetTextColor

LoopRow265:
	mov al, mapwdots26[esi]
	call Writechar
	inc esi
	cmp esi, 19
	jl LoopRow265

mov eax, 1
call SetTextColor

LoopRow266:
	mov al, mapwdots26[esi]
	call Writechar
	inc esi
	cmp esi, 21
	jl LoopRow266

mov eax, 15
call SetTextColor

LoopRow267:
	mov al, mapwdots26[esi]
	call Writechar
	inc esi
	cmp esi, 27
	jl LoopRow267

mov eax, 1
call SetTextColor
mov al, mapwdots26[esi]
call Writechar
call Crlf

;Print row 27

mov esi, 0

mov al, mapwdots27[esi]
call Writechar
inc esi
mov eax, 15
call SetTextColor
mov al, mapwdots27[esi]
call Writechar
inc esi
mov eax, 1
call SetTextColor

LoopRow271:
	mov al, mapwdots27[esi]
	call Writechar
	inc esi
	cmp esi, 12
	jl LoopRow271

mov eax, 15
call SetTextColor
mov al, mapwdots27[esi]
call Writechar
inc esi
mov eax, 1
call SetTextColor

LoopRow272:
	mov al, mapwdots27[esi]
	call Writechar
	inc esi
	cmp esi, 15
	jl LoopRow272

mov eax, 15
call SetTextColor
mov al, mapwdots27[esi]
call Writechar
inc esi
mov eax, 1
call SetTextColor

LoopRow273:
	mov al, mapwdots27[esi]
	call Writechar
	inc esi
	cmp esi, 26
	jl LoopRow273

mov eax, 15
call SetTextColor
mov al, mapwdots27[esi]
call Writechar
inc esi
mov eax, 1
call SetTextColor
mov al, mapwdots27[esi]
call Writechar
call Crlf

;Print row 28

mov esi, 0

mov al, mapwdots28[esi]
call Writechar
inc esi
mov eax, 15
call SetTextColor
mov al, mapwdots28[esi]
call Writechar
inc esi
mov eax, 1
call SetTextColor

LoopRow281:
	mov al, mapwdots28[esi]
	call Writechar
	inc esi
	cmp esi, 12
	jl LoopRow281

mov eax, 15
call SetTextColor
mov al, mapwdots28[esi]
call Writechar
inc esi
mov eax, 1
call SetTextColor

LoopRow282:
	mov al, mapwdots28[esi]
	call Writechar
	inc esi
	cmp esi, 15
	jl LoopRow282

mov eax, 15
call SetTextColor
mov al, mapwdots28[esi]
call Writechar
inc esi
mov eax, 1
call SetTextColor

LoopRow283:
	mov al, mapwdots28[esi]
	call Writechar
	inc esi
	cmp esi, 26
	jl LoopRow283

mov eax, 15
call SetTextColor
mov al, mapwdots28[esi]
call Writechar
inc esi
mov eax, 1
call SetTextColor
mov al, mapwdots28[esi]
call Writechar
call Crlf

;Print row 29

mov esi, 0

mov al, mapwdots29[esi]
call Writechar
inc esi
mov eax, 15
call SetTextColor

LoopRow291:
	mov al, mapwdots29[esi]
	call Writechar
	inc esi
	cmp esi, 27
	jl LoopRow291

mov eax, 1
call SetTextColor
mov al, mapwdots29[esi]
call Writechar
call Crlf

;Print row 30

mov eax, 1
call SetTextColor
mov edx, OFFSET mapwdots30
call WriteString
call Crlf

RET
PrintMapWithDots ENDP

UpdateChamp PROC
	call ReadKey			;readkey is in the irvine32 library and it performs a no-wait check to the state of the keyboard-
							; -and returns the ascii value of the key to ax, as well as the scancode to dx and one other, idk its in the book and I didn't use it
	jz skip					;Also it returns zero if there is no key pressed so just skip and rewrite the map/champ
	mov char, al


	;Quit (Q or q)
	cmp char, 81
	je quit_pressed
	cmp char, 113
	je quit_pressed

	;Pause (P or p)
	cmp char, 80
	je pause_pressed
	cmp char, 112
	je pause_pressed
	
	;Help (H or h)
	cmp char, 'H'
	je help_pressed
	cmp char, 'h'
	je help_pressed

	;left (A or a)
	cmp char, 97
	je left_pressed
	cmp char, 65
	je left_pressed

	;right (D or d)
	cmp char, 68
	je right_pressed
	cmp char, 100
	je right_pressed

	;up (W or w)
	cmp char, 87
	je up_pressed
	cmp char, 119
	je up_pressed

	;down (S or s)
	cmp char, 83
	je down_pressed
	cmp char, 115
	je down_pressed

	jmp skip

	left_pressed:
		dec champ_ynew
		mov champ, '>'						;changes pacman's "sprite" if he changes direction
		jmp skip
	right_pressed:
		inc champ_ynew
		mov champ, '<'
		jmp skip
	up_pressed:
		dec champ_xnew
		mov champ, 'v'
		jmp skip
	down_pressed:
		inc champ_xnew
		mov champ, '^'
		jmp skip
	quit_pressed:
		mov champ_x, -1
		jmp skip
	pause_pressed:
		cmp pauseme, 0
		je pauseit
		jne unpauseit
		pauseit:
			mov pauseme, 1
			jmp skip
		unpauseit:
			mov pauseme, 0
			jmp skip
	help_pressed:
		;IMPLEMENT
		jmp skip

	skip:
		ret
UpdateChamp ENDP

UpdatePause PROC
	call ReadKey
	mov char, al
	
	;Pause (P or p)
	cmp char, 80
	je Pause_pressed
	cmp char, 112
	je Pause_pressed
	
	jmp Skip

	Pause_pressed:
		cmp pauseme, 0
		je PauseIt
		jne UnpauseIt
		PauseIt:
			mov pauseme, 1
			jmp Skip
		UnpauseIt:
			mov pauseme, 0
			jmp Skip
	Skip:

	ret
UpdatePause ENDP

WriteChamp PROC
	AND eax, 0
	AND edx, 0
	mov dh, champ_x
	mov dl, champ_y
	call GoToXY
	mov eax, 14
	call SetTextColor
	mov al, champ
	call WriteChar
	ret
WriteChamp ENDP

WheresChamp PROC				;Debug method, maybe re-enabled in Durga mode or something. Displays coordinates of pac as well as x/ynew in top left
	mov dx, 0
	call GoToXY
	AND eax, 0
	mov al, champ_x
	call WriteInt
	call CRLF
	mov al, champ_y
	call WriteInt
	call CRLF
	AND eax, 0
	mov al, champ_xnew
	call WriteInt
	call CRLF
	AND eax, 0
	mov al, champ_ynew
	call WriteInt
	call CRLF
	ret
WheresChamp ENDP

CheckMove PROC
	;check move happens between UpdateChamp and WriteChamp to see if pacman should be written to the new location or not.

	;I used a macro that accepts champ_y and the appropriate string to check (mapwdotsX, 0 <= X <= 30) that tests [mapwdotsX+champ_ynew] to see what character is there
	;more on that at implementation
	
	cmp champ_xnew, -1				;disallow negative values for champ_x
	jle skip
	cmp champ_ynew, -1				;if ynew is -1, test for special case tunnel on left
	jz testxleft
	cmp champ_xnew, 31				;champ_x cannot exceed 29
	jge skip
	cmp champ_ynew, 28				;if ynew is 28, test for special case tunnel on right
	jz testxright
	jg skip

	cmp champ_xnew, 10				;checks champ_xnew against 10 to jump over some checks to speed things up
	ja oneThird
	cmp champ_xnew, 20				;checks again to potentially skip 10 more checks
	ja twoThirds

	cmp champ_xnew, 0				;Big section of cmp-jcond pairs that tell the program what row pacman is in
	jz Row0
	cmp champ_xnew, 1
	jz Row1
	cmp champ_xnew, 2
	jz Row2
	cmp champ_xnew, 3
	jz Row3
	cmp champ_xnew, 4
	jz Row4
	cmp champ_xnew, 5
	jz Row5
	cmp champ_xnew, 6
	jz Row6
	cmp champ_xnew, 7
	jz Row7
	cmp champ_xnew, 8
	jz Row8
	cmp champ_xnew, 9
	jz Row9
	cmp champ_xnew, 10
	jz Row10
oneThird:
	cmp champ_xnew, 11
	jz Row11
	cmp champ_xnew, 12
	jz Row12
	cmp champ_xnew, 13
	jz Row13
	cmp champ_xnew, 14
	jz Row14
	cmp champ_xnew, 15
	jz Row15
	cmp champ_xnew, 16
	jz Row16
	cmp champ_xnew, 17
	jz Row17
	cmp champ_xnew, 18
	jz Row18
	cmp champ_xnew, 19
	jz Row19
	cmp champ_xnew, 20
	jz Row20
twoThirds:
	cmp champ_xnew, 21
	jz Row21
	cmp champ_xnew, 22
	jz Row22
	cmp champ_xnew, 23
	jz Row23
	cmp champ_xnew, 24
	jz Row24
	cmp champ_xnew, 25
	jz Row25
	cmp champ_xnew, 26
	jz Row26
	cmp champ_xnew, 27
	jz Row27
	cmp champ_xnew, 28
	jz Row28
	cmp champ_xnew, 29
	jz Row29
	cmp champ_xnew, 30
	jz Row30

;contents of mCheckrow:
	;the macro works by pasting the following code into the program in place of the line "mCheckrow champ_ynew, mapwdotsX, qX"

;mCheckrow MACRO champ_ynew, rowtocheck, loopname
	;mov esi, OFFSET rowtocheck				;;rowtocheck is replaced with "mapwdotsX"
	;inc champ_ynew							;;couldn't do "mov ecx, champ_ynew+1" so this was the next best thing
	;mov ecx, champ_ynew					;;this loops through the loop (champ_ynew+1) times, in order to get the value from esi for appropriate location
	;loopname:								;;loopname exists so that there is no overloading of the same label (because a macro is basically a big TEXTEQU)
	;	mov al, [esi]						;;get character from esi into al
	;	inc esi
	;LOOP loopname
	;dec champ_ynew							;;so as not to mess with actual move
	;cmp al, '#'							;;Walls are #
	;jz isWall
	;cmp al, '-'							;;Ghost-pen walls are walls too
	;jz isWall
	;cmp al, '.'							;;A period represents a pellet to eat for points
	;jz isPellet
	;cmp al, '*'							;;An asterisk represents a power pellet
	;jz isPowerPellet
	;cmp al, '@'							;;An @ represents a ghost. 2spooky4me
	;jz isGhost
	;cmp al, '%'							;;A % represents a bonus fruit
	;jz isBonus
	;jmp isntWall
;ENDM
	
	Row0:
		mCheckRow champ_ynew, mapwdots0, q0
	Row1:
		mCheckRow champ_ynew, mapwdots1, q1
	Row2:
		mCheckRow champ_ynew, mapwdots2, q2
	Row3:
		mCheckRow champ_ynew, mapwdots3, q3
	Row4:
		mCheckRow champ_ynew, mapwdots4, q4
	Row5:
		mCheckRow champ_ynew, mapwdots5, q5
	Row6:
		mCheckRow champ_ynew, mapwdots6, q6
	Row7:
		mCheckRow champ_ynew, mapwdots7, q7
	Row8:
		mCheckRow champ_ynew, mapwdots8, q8
	Row9:
		mCheckRow champ_ynew, mapwdots9, q9
	Row10:
		mCheckRow champ_ynew, mapwdots10, q10
	Row11:
		mCheckRow champ_ynew, mapwdots11, q11
	Row12:
		mCheckRow champ_ynew, mapwdots12, q12
	Row13:
		mCheckRow champ_ynew, mapwdots13, q13
	Row14:
		mCheckRow champ_ynew, mapwdots14, q14
	Row15:
		mCheckRow champ_ynew, mapwdots15, q15
	Row16:
		mCheckRow champ_ynew, mapwdots16, q16
	Row17:
		mCheckRow champ_ynew, mapwdots17, q17
	Row18:
		mCheckRow champ_ynew, mapwdots18, q18
	Row19:
		mCheckRow champ_ynew, mapwdots19, q19
	Row20:
		mCheckRow champ_ynew, mapwdots20, q20
	Row21:
		mCheckRow champ_ynew, mapwdots21, q21
	Row22:
		mCheckRow champ_ynew, mapwdots22, q22
	Row23:
		mCheckRow champ_ynew, mapwdots23, q23
	Row24:
		mCheckRow champ_ynew, mapwdots24, q24
	Row25:
		mCheckRow champ_ynew, mapwdots25, q25
	Row26:
		mCheckRow champ_ynew, mapwdots26, q26
	Row27:
		mCheckRow champ_ynew, mapwdots27, q27
	Row28:
		mCheckRow champ_ynew, mapwdots28, q28
	Row29:
		mCheckRow champ_ynew, mapwdots29, q29
	Row30:
		mCheckRow champ_ynew, mapwdots30, q30

	testxright:								;tests for special case tunnel on right side
		cmp champ_xnew, 14
		jnz skip
		mov champ_y, 0						;send pac to opposite side if in appropriate row
		jmp skip

	testxleft:								;tests for special case tunnel on left side
		cmp champ_xnew, 14
		jnz skip
		mov champ_y, 27						;send pac to opposite side again
		jmp skip

	isWall:									;these labels all catch jumps that come from mCheckRow
		jmp skip							;If a wall, then just skip and exit without changing champ_x/y

	isntWall:
		mov al, champ_xnew					;if not a wall, move is legal and thus champ_x = champ_xnew, champ_y = champ_ynew
		mov bl, champ_ynew
		mov champ_x, al
		mov champ_y, bl
		jmp skip

	isPellet:								;if pellet, then move is legal, x/y updated, '.' removed from string for that row, and score updated appropriately
		mov cl, ' '
		mov [esi-1], cl
		mov al, champ_xnew
		mov bl, champ_ynew
		mov champ_x, al
		mov champ_y, bl
		mov cx, scoreMod
		add score, cx
		inc dots
		jmp skip

	isPowerPellet:							;if Power Pellet, then move is legal, x/y updated, * replaced with ' ' in appropriate row.
		mov cl, ' '
		mov [esi-1], cl
		add score, 50
		mov al, champ_xnew
		mov bl, champ_ynew
		mov champ_x, al
		mov champ_y, bl
		cmp powermode, 0
		jz powerOn
		jnz powerAlreadyOn

		powerOn:
			mov powermode, 1
			mov bl, POWER_MAX
			mov powercount, bl
		powerAlreadyOn:
			mov bl, POWER_MAX
			mov powercount, bl
		jmp skip

	isGhost:
		;implement pacman death, lives--, score-something, and reset position after death screen
		;also potentially implement ghost eating cmp powermode, 1 jz killghost jnz killthepacman
		cmp powermode, 0
		jz killthepacman
		jnz killghost

		killthepacman:
			dec lives						;minus 1 life for dying obv
			mov champ_x, 23					;reset to starting position
			mov champ_y, 14
		jmp skip

		killghost:
			mov cl, ' '
			mov [esi-1], cl
			mov al, champ_xnew
			mov bl, champ_ynew
			mov champ_x, al
			mov champ_y, bl
		jmp skip
	isBonus:
		mov cl, ' '
		mov [esi-1], cl
		mov al, champ_xnew
		mov bl, champ_ynew
		mov champ_x, al
		mov champ_y, bl
		mov dx, bonus
		add score, dx
		jmp skip

	skip: ret
CheckMove ENDP

PauseScreen PROC
	mov dh, 14
	mov dl, 7
	call GOTOXY
	mov edx, OFFSET pausescreen_msg
	call WriteString
	ret
PauseScreen ENDP

CheckDots PROC
	cmp dots, 70
	jz spawnbonus
	cmp dots, 170
	jz spawnbonus
	
	jmp skip

	spawnbonus:
		mov cl, '%'
		cmp level, 1
		jz level1
		cmp level, 2
		jz level2
		cmp level, 3
		jz level3
		
		level1:
			mov [mapwdots11+14], cl
			mov bonus, 100
			jmp skip
		level2:
			mov [mapwdots11+14], cl
			mov bonus, 300
			jmp skip
		level3:
			mov [mapwdots11+14], cl
			mov bonus, 500
			jmp skip
	skip:
	ret
CheckDots ENDP

CheckScore PROC
	cmp score, 10000
	jge bonuslife
	jmp hasit

	bonuslife:
		cmp hasbonuslife, 1
		jz hasit
		inc lives
		mov hasbonuslife, 1
	hasit:
	ret
CheckScore ENDP


END main